...
class Client:
    ...
    def stream_events(
        ...
        # with requests.post(
        #     self.url, json=data, headers=headers, stream=True
        # ) as response:
        ...
        event = {'messages': [{'role': 'user', 'parts': [{'text': 'Hey'}]}], 'user_id': 'my_user', 'session_id': '2950195c-d64c-42c8-a697-d3fd8558d7b4'}
        yield event
...
class EventProcessor:
    ...
    def __init__(self, st: Any, client: Client, stream_handler: StreamHandler) -> None:
        ...
        self.parts: Part = []
        self.final_content: Content = None
    ...
    def process_events(self) -> None:
        ...
        messages = [message.to_dict() for message in messages] # serializing
    ...
    def handle_chat_model_stream(self, event: Dict[str, Any]) -> None:
        ...
        if content and len(content.strip()) > 0:
            content_part = Part.from_text(content)
            self.parts.append(content_part)
    ...
    def handle_end(self, event: Dict[str, Any]) -> None:
        ...
        self.final_content = Content(role="model", parts=self.parts)
        final_message = self.final_content
